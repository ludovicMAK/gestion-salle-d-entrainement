...
router.post("/", async (req, res) => {
  try {
    const { nom, description, musclesCibles } = req.body;
    
    // Validation des données requises
    if (!nom || !description || !musclesCibles || !Array.isArray(musclesCibles)) {
      return res.status(400).json({ 
        error: "Le nom, la description et les muscles ciblés sont requis" 
      });
    }
